	...
	<div class="discord-inner">
		...
		<div class="dc-header">
			...
			<div>
				<div class="dc-server-name">
					{loading
						? "Loading..."
						: (widgetData?.name ?? "ALTER EGO Wiki Discord")}
				</div>
				<div class="dc-server-sub">
					We are the proudly delusional, insane, and asinine patients
					of Angel Wing Hospital!
				</div>
			</div>
		</div>
		...
		{:else if failed || !widgetData}
			<p class="dc-fallback-text">
				Join our Discord to chat with other fans, get updates, and help
				build the ultimate ALTER EGO resource!
			</p>
		{:else}
			<!-- Stats -->
			<div class="dc-stats">
		...
		<a
			href="https://discord.com/invite/{INVITE_CODE}"
			target="_blank"
			rel="noopener noreferrer"
			class="dc-join-btn"
		>
			<svg
				class="dc-discord-icon"
...
<style>
	...
	.dot-online {
		background: oklch(78% 0.22 145);
	}
	.dot-all {
		background: oklch(78% 0.02 240);
	}
	...
	.dc-member-dot {
		...
		&.idle {
			background: oklch(85% 0.18 85);
		}
		&.dnd {
			background: oklch(60% 0.2 15);
		}
		&.offline {
			background: oklch(52% 0.03 240);
		}
	}
